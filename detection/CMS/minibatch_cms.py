...
class CMSRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes=None, gt_ishard=None, dontcare_areas=None):
        ...
        x1_rpn = nn.MaxPool2d(2)(x1)

        x2 = self.seq2(x1_rpn)
        x2_rpn = nn.MaxPool2d(2)(x2)
        ...
        # 2x pooling
        x1_rpn = self.l2norm_rpn_1(nn.MaxPool2d(2)(x1_rpn))
        # 1x pooling
        ...
        x1_rpn = self.roi_pool1(x1, body[0]).unsqueeze(0)
        x2_rpn = self.roi_pool2(x2, body2[0]).unsqueeze(0)
        x3_rpn = self.roi_pool3(x3, body3[0]).unsqueeze(0)
        for i in range(1, batch_size):
            x1_rpn = torch.cat((x1_rpn, self.roi_pool1(x1, body[i]).unsqueeze(0)))
            x2_rpn = torch.cat((x2_rpn, self.roi_pool2(x2, body[i]).unsqueeze(0)))
        ...
        # TODO: add dropout layer?
        x1_rpn = self.roi_pool1(x1, face[0]).unsqueeze(0)
        x2_rpn = self.roi_pool2(x2, face[0]).unsqueeze(0)
        x3_rpn = self.roi_pool3(x3, face[0]).unsqueeze(0)
        for i in range(1, batch_size):
            x1_rpn = torch.cat((x1_rpn, self.roi_pool1(x1, face[i]).unsqueeze(0)))
            x2_rpn = torch.cat((x2_rpn, self.roi_pool2(x2, face[i]).unsqueeze(0)))
