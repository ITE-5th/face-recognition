...
class CMSRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes=None, gt_ishard=None, dontcare_areas=None):
        ...
        if self.training:
            ...
            roi_datas = []
            roi_data = self.proposal_target_layer(rois[0], gt_boxes[0], gt_ishard, dontcare_areas, self.n_classes)
            roi_datas.append(roi_data)
            roiss = roi_data[0].unsqueeze(0)

            for i in range(1, batch_size):
                roi_data = self.proposal_target_layer(rois[i], gt_boxes[i], gt_ishard, dontcare_areas, self.n_classes)
                roi_datas.append(roi_data)
        ...
        # MINI-BATCH: it doesnt support mini-batch
        # for-loop here and collect output
        x1_rpn = self.roi_pool3(x1, body[0]).unsqueeze(0)
        x2_rpn = self.roi_pool3(x2, body2[0]).unsqueeze(0)
        x3_rpn = self.roi_pool3(x3, body3[0]).unsqueeze(0)
        for i in range(1, batch_size):
            x1_rpn = torch.cat((x1_rpn, self.roi_pool3(x1, body[i]).unsqueeze(0)))
            x2_rpn = torch.cat((x2_rpn, self.roi_pool3(x2, body[i]).unsqueeze(0)))
            x3_rpn = torch.cat((x3_rpn, self.roi_pool3(x3, body[i]).unsqueeze(0)))

        x1_rpn = self.combine_batch_with_rois(x1_rpn)
        x2_rpn = self.combine_batch_with_rois(x2_rpn)
        x3_rpn = self.combine_batch_with_rois(x3_rpn)
        ...
        # MINI-BATCH: roi-pooling doesnt support mini-batches as I see
        # for-loop n collect outputs to solve it
        # TODO: add dropout layer?

        x1_rpn = self.roi_pool3(x1, face[0]).unsqueeze(0)
        x2_rpn = self.roi_pool3(x2, face[0]).unsqueeze(0)
        x3_rpn = self.roi_pool3(x3, face[0]).unsqueeze(0)
        for i in range(1, batch_size):
            x1_rpn = torch.cat((x1_rpn, self.roi_pool3(x1, face[i]).unsqueeze(0)))
            x2_rpn = torch.cat((x2_rpn, self.roi_pool3(x2, face[i]).unsqueeze(0)))
            x3_rpn = torch.cat((x3_rpn, self.roi_pool3(x3, face[i]).unsqueeze(0)))

        x1_rpn = self.combine_batch_with_rois(x1_rpn)
        x2_rpn = self.combine_batch_with_rois(x2_rpn)
        x3_rpn = self.combine_batch_with_rois(x3_rpn)

        face = torch.cat((self.l2norm_faces_1(x1_rpn),
                          self.l2norm_faces_2(x2_rpn),
                          self.l2norm_faces_2(x3_rpn)), 1)

        face = self.conv1x1_rpn(face)  # rois ( rois_count , 5)
        ...
        if self.training:
            ...
            # MINI-BATCH: check loss
            self.cross_entropy, self.loss_box = 0, 0
            for i in range(batch_size):
                ce, lb = self.build_loss(cls_prob[i], bbox_pred[i], roi_datas[i])
            ...
            # self.cross_entropy, self.loss_box = self.build_loss(cls_prob, bbox_pred, roi_datas)
