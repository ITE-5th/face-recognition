...
class CMSRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes=None, gt_ishard=None, dontcare_areas=None):
        ...
        face, body = self.project_into_feature_maps(x1, roiss, im_info)
        face2, body2 = self.project_into_feature_maps(x2, roiss, im_info)
        face3, body3 = self.project_into_feature_maps(x3, roiss, im_info)

        x1_rpn = self.roi_pool1(x1, body[0])
        x2_rpn = self.roi_pool2(x2, body2[0])
        x3_rpn = self.roi_pool3(x3, body3[0])
        for i in range(1, batch_size):
            x1_rpn = torch.cat((x1_rpn, self.roi_pool1(x1, body[i])))
            x2_rpn = torch.cat((x2_rpn, self.roi_pool2(x2, body[i])))
            x3_rpn = torch.cat((x3_rpn, self.roi_pool3(x3, body[i])))
        ...
        body = self.conv1x1_rpn(body)
        body = body.view(body.size()[0], -1)

        # TODO: add dropout layer?
        x1_rpn = self.roi_pool1(x1, face[0])
        x2_rpn = self.roi_pool2(x2, face[0])
        x3_rpn = self.roi_pool3(x3, face[0])
        for i in range(1, batch_size):
            x1_rpn = torch.cat((x1_rpn, self.roi_pool1(x1, face[i])))
            x2_rpn = torch.cat((x2_rpn, self.roi_pool2(x2, face[i])))
            x3_rpn = torch.cat((x3_rpn, self.roi_pool3(x3, face[i])))
        ...
        if self.training:
            # cls_prob ( rois_count, 2 )
            # bbox_pred ( rois_count, 8)
    ...
    def project_into_feature_maps(feature_maps, rois, im_info):
        ...
        # TODO: taken by hand using GIMP, double check!
