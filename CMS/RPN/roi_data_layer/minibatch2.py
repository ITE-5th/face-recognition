...
def _get_viewpoint_estimation_labels(viewpoint_data, clss, num_classes):
    ...
    inds = np.where((clss > 0) & np.isfinite(viewpoint_data[:, 0]) & np.isfinite(viewpoint_data[:, 1]) & np.isfinite(
        viewpoint_data[:, 2]))[0]
    for ind in inds:
        cls = clss[ind]
        start = 3 * cls
...
def _vis_minibatch(im_blob, rois_blob, labels_blob, overlaps, sublabels_blob, view_targets_blob=None,
                   view_inside_blob=None):
